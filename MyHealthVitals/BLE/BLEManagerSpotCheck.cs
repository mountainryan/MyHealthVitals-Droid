...
{
	public class BleManagerSpotCheck
	{
		public static ICharacteristic bmChar;
		public static IDevice connectedSpotCheck;

		public static IAdapter Adapter;
		public static IBluetoothCallBackUpdatable uiController;

		public void ScanToConnectToSpotCheck(IBluetoothCallBackUpdatable controller)
		{
		...
		public void executeWrieCommand(byte[] byteCommand)
		{
			if (connectedSpotCheck != null && connectedSpotCheck.State == Plugin.BLE.Abstractions.DeviceState.Connected)
			{
		...
		{
			executeWrieCommand(new byte[] { 0xaa, 0x55, 0x40, 0x02, 0x01, 0x29 });
		}

		public void stopReadingECG()
		{
			executeWrieCommand(new byte[] { 0xaa, 0x55, 0x30, 0x02, 0x02, 0x2 });
		}

		public void startEcgMeasuring()
		{
			executeWrieCommand(new byte[] { 0xaa, 0x55, 0x31, 0x02, 0x02, 0xC6 });
		}
		...
		void Adapter_DeviceConnectionLost(object sender, Plugin.BLE.Abstractions.EventArgs.DeviceErrorEventArgs e)
		{
			if (e.Device.Name == "PC_300SNT")
			{
				...
				//connectedSpotCheck.
				//connectedSpotCheck.
				//connectedSpotCheck.Dispose();
				//ada

				//Adapter.DisconnectDeviceAsync(connectedSpotCheck);
				Debug.WriteLine("PC_300SNT just disconnected");
				uiController.ShowMessageOnUI("Disconnected.", false);
			}
		}
		...
		void Adapter_ScanTimeoutElapsed(object sender, EventArgs e)
		{
			...
			uiController.ShowMessageOnUI("Scanning time out. Please, check if device is turned on.", false);
		}
		...
		private void C_ValueUpdated(object sender, Plugin.BLE.Abstractions.EventArgs.CharacteristicUpdatedEventArgs e)
		{
			...
			if ((int)ch.Value[2] >= 48 && (int)ch.Value[2] <= 51)
			{
				...
				//	//	BluetoothCentralManager.connectedSpotCheck.WriteValue(NSData.FromArray(bytes), ((BluetoothPeripheralDelegate)BluetoothCentralManager.connectedSpotCheck.Delegate).bmChar, CBCharacteristicWriteType.WithResponse);
				//	//}
				//	//else {
